...
const PDFManagement: React.FC = () => {
  ...
  const handleSubmitPdf = async (e: React.FormEvent) => {
    ...
    try {
      let coverUrl = pdfForm.cover_url;
      ...
      if (pdfFile) {
        ...
        fileUrl = await uploadFile(pdfFile, 'pdf-files');
      ...
      if (editingItem) {
        await updatePDFFile(editingItem.id, updatedPdfData);
      } else {
        await addPDFFile(updatedPdfData);
      }
      ...
      addToast({
        type: 'success',
        title: editingItem ? 'PDF Updated' : 'PDF Added',
        message: `PDF ${editingItem ? 'updated' : 'added'} successfully`,
        duration: 3000
      });
    } catch (error) {
      ...
      addToast({
        type: 'error',
        title: 'Error',
        message: 'Gagal menyimpan PDF. Silakan coba lagi.',
        duration: 5000
      });
    } finally {
  ...
  const handleDeletePdf = async (pdfId: number) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus PDF ini?')) {
      try {
  ...
  const getPaginatedData = () => {
    ...
    return filteredPdfFiles.slice(startIndex, endIndex);
  };

  const getTotalPages = () => {
    return Math.ceil(filteredPdfFiles.length / itemsPerPage);
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mt-6 space-y-4 sm:space-y-0">
        <div className="flex items-center space-x-4">
          <span className="text-sm text-gray-700">
            Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, filteredPdfFiles.length)} of {filteredPdfFiles.length} files
          </span>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Manajemen PDF</h2>
          <p className="text-gray-600">Upload dan kelola file PDF untuk pengguna</p>
        </div>
        <button
            ...
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold text-gray-900">
                  {editingItem ? 'Edit PDF' : 'Upload PDF'}
                </h3>
                <button
                  onClick={() => {
                    setShowAddForm(false);
              ...
              <form onSubmit={handleSubmitPdf} className="space-y-6">
                <div>
                ...
                <div>
                  ...
                  <select
                    value={pdfForm.categoryId}
                    onChange={(e) => setPdfForm(prev => ({ ...prev, categoryId: Number(e.target.value) }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                    required
                  >
                ...
                <div className="flex justify-end space-x-3 pt-6 border-t">
                  ...
                  <button
                    ...
                    className="flex items-center space-x-2 px-6 py-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-lg hover:from-green-600 hover:to-emerald-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isSubmitting ? (
                      <>
                      ...
                      <>
                        <Upload className="w-4 h-4" />
                        <span>{editingItem ? 'Update' : 'Upload'} PDF</span>
                      </>
                    )}
                  </button>
                </div>
